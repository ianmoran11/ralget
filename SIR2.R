...
S00 <-   v("S00",   .func = const(c(10000, 10000)))
I00 <-   v("I00",   .func = const(c(10,10)))
R00 <-   v("R00",   .func = const(c(0,0)))
S01 <-   v("S01",   .func = sum)
I01 <-   v("I01",   .func = sum)
R01 <-   v("R01",   .func = sum)
...
I02 <-   v("I02",   .func = sum)
R02 <-   v("R02",   .func = sum)
B <- v("B", .func = const(rnorm(2,.3/10010,.1/10010)))
G <- v("G", .func = const(rnorm(2,500/10010, 1000/10010)))
...
SIR_FULL %>% get_edge_names() %>% activate("nodes") %>% evaluate() %>%
as_tibble() %>% 
mutate(id = row_number()) %>% unnest(eval_statement) %>%
group_by(id) %>% mutate(sim = row_number()) %>% 
select(name, r = eval_statement,sim) %>% 
mutate(t = str_extract(name, "[0-9]+")) %>% 
mutate(name = str_remove(name, "[0-9]+")) %>% 
spread(name,r)

A2 %>% gather(key,value, -t, -sim) %>% 
ggplot(aes(x = as.numeric(t),y = value,group = sim,  color = key )) + 
geom_line() + 
geom_point() + facet_wrap(~sim)